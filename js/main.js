...
(function () {
  ...
  const navbarlinksActive = () => {
    const currentPosition = window.pageYOffset - 1;
...
const on = (type, el, listener, all = false) => {
  let elements = all ? selectAll(el) : [select(el)];
...
let navbarLinks = selectAll("#navbar .scrollto");
...
if (typed) {
  let typedStrings = [...typed.dataset.strings.split(",")];
  new Typed(typed, {
